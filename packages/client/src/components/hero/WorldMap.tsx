...
// Each [x, y] represents a point on the continent outlines
...
// Featured destination approximate coordinates for highlight dots
  ...
  return (
    ...
    >
      ...
      {FEATURED_CITIES.map(([x, y], i) => (
        <g key={`city-${i}`}>
          <circle cx={x} cy={y} r={3} className="fill-aurora/40">
            {!shouldReduceMotion && (
              <animate
                attributeName="r"
                values="3;6;3"
          ...
          <circle cx={x} cy={y} r={2} className="fill-aurora-light" />
        </g>
      ))}
      ...
      <defs>
        <linearGradient id="pathGradient" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#3377ff" stopOpacity={0.8} />
